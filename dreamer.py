...
def make_env(config, logger, mode, train_eps, eval_eps):
    env = make_safety(config.task, action_repeat=config.action_repeat)
...
def main(config):
    wandb_name = str(config.task) + "_" + str(config.lag_clip)
    wandb.login()
    wandb.init(project="dreamer2", entity="joshuaohara71", name=wandb_name)
