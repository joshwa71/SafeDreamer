...
class ImagBehavior(nn.Module):
    ...
    def update_lag(self):
        ...
        loss_lag.backward()
        torch.nn.utils.clip_grad_norm_(self.raw_lag, self._config.lag_grad_clip)
        self.optim_lagrange.step()
        with torch.no_grad():
            self.lagrange = torch.nn.functional.softplus(self.raw_lag)
        clone = self.raw_lag.clone().detach()
        clamped = torch.clamp(clone, min=-self._config.lag_clip, max=self._config.lag_clip)
        self.raw_lag = torch.tensor(clamped, requires_grad=True).to(self._config.device)
        self.optim_lagrange = SGD([self.raw_lag], self._config.lag_lr)
